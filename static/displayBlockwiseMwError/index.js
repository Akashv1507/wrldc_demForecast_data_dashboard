$(document).ready(function () {
  ...
  var multipleRevisionButton = new Choices("#revisionNo", {
    removeItemButton: true,
    maxItemCount: 8,
    searchResultLimit: 8,
    renderChoiceLimit: 8,
  ...
  $("#blockwiseMwwiseError thead tr")
    .clone(true)
    .appendTo("#blockwiseMwwiseError thead");
  $("#blockwiseMwwiseError thead tr:eq(1) th").each(function (i) {
    var title = $(this).text();
  ...
  //for dataTable
  var table = $("#blockwiseMwwiseError").DataTable({
    dom: "Bfrtip",
...
function validateForm() {
  ...
  for (var option of document.getElementById("revisionNo").options) {
    if (option.selected) {
      selectedRevisinoList.push(option.value);
    }
  }
  ...
  if (selectedRevisinoList.length == 0) {
    errorDiv.innerHTML =
      "<b>Error !!!! Select Revision No. from dropdown </b> ";
    return false;
  }
